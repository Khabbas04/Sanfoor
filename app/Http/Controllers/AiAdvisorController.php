...
class AiAdvisorController extends Controller
{
    ...
    private function buildSystemPrompt($user, array $academicData, array $cartData, array $availableCourses, string $ragContext = ''): string
    {
        ...
        return "أنت مرشد أكاديمي ذكي لتطبيق 'سنفور' الخاص بطلاب جامعة الزرقاء. مهمتك الإجابة على استفسارات الطلاب بناءً على سياق الـ RAG المقدم لك فقط.\nالقواعد:\n" .
            "- كن مختصراً جداً ومباشراً (Direct).\n" .
            "- لا تكرر الأسئلة في إجابتك.\n" .
            "- إذا سأل الطالب عن متطلبات مادة، اذكرها كنقاط فقط.\n" .
            "- لا تستخدم جمل ترحيبية طويلة في كل مرة.\n" .
            "- استخدم ايموجيات خفيفة وميّز الكلمات المهمة بالخط العريض (**bold**).\n" .
            "- إذا كانت المعلومات غير متوفرة في السياق، اطلب من الطالب مراجعة التسجيل.\n\n" .
