...
class AiAdvisorController extends Controller
{
    ...
    public function chatStream(Request $request)
    {
        ...
        if (!$this->checkRateLimit($user->id)) {
            return response()->json([
                'status' => 'success',
                'reply' => '⏳ وصلت للحد الأقصى من الرسائل لهذا الوقت. حاول لاحقاً.',
                'suggested_courses' => [], 'courses_to_remove' => [],
                'follow_up_suggestions' => [], 'interactive_widget' => null,
                'chat_id' => $data['chat_id'] ?? null,
                'daily_messages_remaining' => $dailyLimit === null ? null : max(0, $dailyLimit - $usage),
                'has_daily_limit' => $dailyLimit !== null,
            ]);
        ...
        // Pre-build contents outside the stream if we'll need them
        $contents = null;
        if (!is_array($cachedAiResponse) && !empty($apiKeys)) {
            $ragContext = $this->buildStudentAdvisingRagContext($academicData, $cartData, $availableCourses, $data['message']);
            $systemPrompt = $this->buildSystemPrompt($user, $academicData, $cartData, $availableCourses, $ragContext, $data['filters'] ?? [], $data['difficulty'] ?? null, $data['critical_path'] ?? null);
            $contents = $this->buildConversationContext($chat, $systemPrompt);
        }
        ...
        return response()->stream(function () use ($user, $data, $chat, $chatId, $isNewChat, $academicData, $cartData, $availableCourses, $apiKeys, $cachedAiResponse, $dailyLimit, $usage, $usageKey, $responseCacheKey, $contents) {
            ...
            if (is_array($cachedAiResponse) && isset($cachedAiResponse['reply'])) {
                ...
                $newRemaining = $dailyLimit !== null ? max(0, $dailyLimit - $usage) : null;
            ...
            // === NO API KEYS — local fallback ===
            if (empty($apiKeys)) {
                try {
                    $parsed = $this->getLocalFallbackResponse($data['message'], $user, $academicData, $cartData, $availableCourses);
                    $chat->messages()->create(['role' => 'ai', 'content' => json_encode($parsed, JSON_UNESCAPED_UNICODE)]);
                    $newRemaining = null;
                    if ($dailyLimit !== null) { Cache::put($usageKey, $usage + 1, now()->endOfDay()); $newRemaining = max(0, $dailyLimit - ($usage + 1)); }

                    $this->sendSSE('done', [
                        'status' => 'success', 'reply' => $parsed['reply'],
                        'suggested_courses' => $parsed['suggested_courses'] ?? [], 'courses_to_remove' => $parsed['courses_to_remove'] ?? [],
                        'follow_up_suggestions' => $parsed['follow_up_suggestions'] ?? [], 'interactive_widget' => $parsed['interactive_widget'] ?? null,
                        'chat_id' => $chatId, 'chat_title' => $isNewChat ? $chat->title : null,
                        'daily_messages_remaining' => $newRemaining, 'has_daily_limit' => $dailyLimit !== null, 'is_fallback' => true,
                    ]);
                } catch (\Throwable $e) {
                    Log::error('Stream fallback error: ' . $e->getMessage());
                    $this->sendSSE('error', ['message' => 'حدث خطأ غير متوقع.']);
                }
                return;
            }
            ...
            // === STREAM FROM GEMINI ===
            try {
                $fullText = $this->callGeminiStreamingAPI($contents, $apiKeys, function ($newReplyText) {
                    $this->sendSSE('token', ['t' => $newReplyText]);
                });
                ...
                $matched = $this->matchCoursesInReply($replyText, $availableCourses['map'], $cartData['map']);
                $suggestedDetails = !empty($matched['suggested']) ? Course::whereIn('id', $matched['suggested'])->select('id', 'name', 'code', 'credit_hours', 'description')->get()->toArray() : [];
                $removeDetails = !empty($matched['remove']) ? Course::whereIn('id', $matched['remove'])->select('id', 'name', 'code', 'credit_hours', 'description')->get()->toArray() : [];

                $chat->messages()->create(['role' => 'ai', 'content' => json_encode([
                    'reply' => $replyText, 'suggested_courses' => $suggestedDetails, 'courses_to_remove' => $removeDetails,
                    'follow_up_suggestions' => $followUpSuggestions, 'interactive_widget' => $interactiveWidget,
                ], JSON_UNESCAPED_UNICODE)]);

                if ($isNewChat) { $chat->update(['title' => $this->makeFallbackTitle($data['message'])]); }
                ...
                $this->sendSSE('done', [
                    'status' => 'success', 'reply' => $replyText,
                    'suggested_courses' => $suggestedDetails, 'courses_to_remove' => $removeDetails,
                    'follow_up_suggestions' => $followUpSuggestions, 'interactive_widget' => $interactiveWidget,
                    'chat_id' => $chatId, 'chat_title' => $isNewChat ? $chat->title : null,
                    'daily_messages_remaining' => $newRemaining, 'has_daily_limit' => $dailyLimit !== null,
                    'is_fallback' => false, 'is_cached' => false,
                ]);
            } catch (\Throwable $e) {
                ...
                try {
                    $parsed = $this->getLocalFallbackResponse($data['message'], $user, $academicData, $cartData, $availableCourses);
                    $replyText = "💡 *(مستشار سنفور البديل)*\n\n" . $parsed['reply'];
                    $chat->messages()->create(['role' => 'ai', 'content' => json_encode(array_merge($parsed, ['reply' => $replyText]), JSON_UNESCAPED_UNICODE)]);
                    $newRemaining = null;
                    if ($dailyLimit !== null) { Cache::put($usageKey, $usage + 1, now()->endOfDay()); $newRemaining = max(0, $dailyLimit - ($usage + 1)); }
                    ...
                    $this->sendSSE('error', ['message' => 'حدث خطأ. حاول مرة أخرى.']);
                }
            }
        }, 200, [
    ...
    private function buildSystemPrompt($user, array $academicData, array $cartData, array $availableCourses, string $ragContext = '', array $filters = [], $difficulty = null, $criticalPath = null): string
    {
        ...
        return "أنت مرشد أكاديمي ذكي لتطبيق 'سنفور' الخاص بطلاب جامعة الزرقاء. دورك: إجابة أسئلة الطلاب عن الإرشاد الأكاديمي والجامعة وكل ما يحتاجه الطالب في مسيرته الدراسية باحتراف وذكاء.\nالقواعد:\n" .
            ...
            "- مباني: أ.ب(الفاروق)، ت(طبية/تقنية)، د.هـ(الدوازي)، ل(هندسة/فنون)، ص(صحافة/حقوق)، ق(اقتصاد)، ط(أسنان). الطوابق: 100=أول، 200=ثاني.\n" .
    ...
    private function extractReplyFromPartialJson(string $buffer): string
    {
        if (!preg_match('/"reply"\s*:\s*"/', $buffer, $m, PREG_OFFSET_CAPTURE)) {
            return '';
        }
        ...
        while ($i < $len) {
            ...
            if ($ch === '\\') {
                if ($i + 1 < $len) {
                    ...
                    break; // Incomplete escape — stop here safely
                }
            } elseif ($ch === '"') {
                break; // End of reply value
            } else {
                $result .= $ch;
                $i++;
            }
    ...
    private function callGeminiStreamingAPI(array $contents, array $apiKeys, callable $onReplyChunk): string
    {
        $model = 'gemini-2.5-flash';
        $sortedKeys = $this->sortKeysByAvailability($apiKeys);
        $lastError = 'No streaming keys available';

        foreach ($sortedKeys as $apiKey) {
            $keyIndex = array_search($apiKey, $apiKeys, true);

            $cooldown = $this->getKeyCooldownRemaining($apiKey);
            if ($cooldown > 0) { $lastError = "key#" . ($keyIndex + 1) . ": cooldown"; continue; }

            $rpm = $this->getKeyRpm($apiKey);
            if ($rpm >= self::RPM_LIMIT) { $lastError = "key#" . ($keyIndex + 1) . ": RPM limit"; continue; }
            ...
            curl_setopt_array($ch, [
                ...
                CURLOPT_HEADERFUNCTION => function ($ch, $header) use (&$curlError, $apiKey, $keyIndex, &$lastError) {
                    if (preg_match('/^HTTP\/\S+\s+(\d{3})/', $header, $m)) {
                        $code = (int) $m[1];
                        if ($code === 429) { $this->setCooldown($apiKey, 60, 'stream_429'); $curlError = true; $lastError = "key#" . ($keyIndex + 1) . ": 429"; }
                        elseif (in_array($code, [401, 403])) { $this->setCooldown($apiKey, 600, 'stream_auth'); $curlError = true; $lastError = "key#" . ($keyIndex + 1) . ": {$code}"; }
                        elseif ($code >= 400) { $curlError = true; $lastError = "key#" . ($keyIndex + 1) . ": HTTP {$code}"; }
                    }
                    return strlen($header);
                },
                CURLOPT_WRITEFUNCTION => function ($ch, $data) use (&$fullText, &$sseBuffer, &$lastSentReplyLen, $onReplyChunk, &$curlError) {
                    if ($curlError) return strlen($data);
                    ...
                    while (($pos = strpos($sseBuffer, "\n")) !== false) {
                        ...
                        if (str_starts_with($line, 'data: ')) {
                            ...
                            if (is_string($text) && $text !== '') {
                                ...
                                $currentReply = $this->extractReplyFromPartialJson($fullText);
                                $currentLen = mb_strlen($currentReply, 'UTF-8');

                                if ($currentLen > $lastSentReplyLen) {
                                    $newChunk = mb_substr($currentReply, $lastSentReplyLen, null, 'UTF-8');
                                    $onReplyChunk($newChunk);
                                    $lastSentReplyLen = $currentLen;
                                }
                            }
                        }
                    }
            ...
            if (!$curlError && $httpCode === 200 && $fullText !== '') {
                $this->incrementKeyRpm($apiKey);
                $this->workingApiKey = $apiKey;
                $dailyKey = 'gemini_key_usage_' . md5($apiKey) . '_' . date('Y-m-d');
                Cache::put($dailyKey, (int) Cache::get($dailyKey, 0) + 1, now()->endOfDay());
            ...
            if (!$curlError) {
                $lastError = "key#" . ($keyIndex + 1) . ": HTTP {$httpCode}";
            }
        }

        throw new \Exception("Streaming failed across all keys. Last: {$lastError}");
    }
    ...
    private function parseAIResponse(string $rawText): array
    {
        ...
        $extracted = $this->extractReplyFromPartialJson($clean);
        if ($extracted !== '') {
            return [
                'reply' => $extracted,
                'follow_up_suggestions' => [],
