...
class AiContextAssembler
{
    ...
    public function build(array $rules, array $rankedCourses, array $ragData, array $documentContext, array $riskWarnings = []): array
    {
        ...
        $systemPrompt .= "🔴 تنبيه هام جداً 🔴: إذا طلب الطالب (إضافة مادة) قم بوضع الـ ID الخاص بها في مصفوفة courses_to_add، وقل له 'تمت إضافتها بنجاح'. إياك أن تخبره أنك لا تستطيع الإضافة!\n";
        $systemPrompt .= "🔴 تنبيه هام جداً 🔴: إذا طلب الطالب رؤية (خطته الشجرية، خريطة المواد، الشجرة)، إياك أن تعتذر أو تقول أنك لا تستطيع الرسم! بل قل 'بالتأكيد، تفضل خطتك:' ثم ضع هذا النص الحرفي: %%SKILL_TREE%%\n";
        $systemPrompt .= "مثال: الطالب: 'ارسم خطتي' -> أنت تجيب: 'تفضل: %%SKILL_TREE%%'.\n\n";
