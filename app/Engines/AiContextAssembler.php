...
class AiContextAssembler
{
    ...
    public function build(array $rules, array $rankedCourses, array $ragData, array $documentContext): array
    {
        ...
        $systemPrompt .= "2. 🔀 **المخططات الانسيابية (Flowcharts):** إذا سألك الطالب بشكل مباشر \"رسم مخطط\"، \"flowchart\"، \"flow chart\"، \"مخطط شجري\"، \"سلسلة المواد\"، أو متطلبات التخرج، **يجب إجبارياً** أن ترسم مخطط انسيابي باستخدام `mermaid`.\n⚠️ **قواعد صارمة لكتابة كود Mermaid:**\n- ضع كل سهم (-->) في سطر منفصل (لا تستخدم الفاصلة المنقوطة ; أبداً)\n- لا تضع أقواس () داخل أسماء العقد، استخدم فقط الأقواس المربعة []\n- لا تضع رقم المادة أو الشرطة - داخل اسم العقدة\n- ابدأ دائماً بـ graph TD في السطر الأول\n- ضع الكود داخل كتلة ```mermaid\nمثال صحيح:\n```mermaid\ngraph TD\nA[برمجة الحاسوب 1] --> B[شبكات الحاسوب]\nA --> C[هياكل البيانات]\nC --> D[قواعد البيانات]\n```\n";
