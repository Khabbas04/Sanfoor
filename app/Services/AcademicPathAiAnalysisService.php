        ...
        try {
            $payload = [
                ...
                'current_semester' => $courses->map(fn (array $course) => [
                    ...
                    'academic_indicators' => $course['academic_indicators'] ?? [],
                ])->values()->all(),
            ...
            $raw = $this->gemini->callGeminiAPI(
                ...
                [
                    ...
                    'systemInstruction' => ['parts' => [['text' =>
                        'أنت محرك التحليل الأكاديمي الذكي داخل منصة سنفور. الخطة المرسلة تم بناؤها والتحقق منها مسبقًا بقواعد الجامعة. '
                        .'حلّل سبب اختيار كل مادة اعتمادًا فقط على: مستوى الصعوبة، الأولوية، المتطلبات، عدد المواد التي تفتحها، '
                        .'والبيانات الأكاديمية التاريخية المرسلة. لا تصف المؤشرات ذات العينة غير الكافية بأنها دراسة مؤكدة، ولا تخترع أبحاثًا أو نسبًا. '
                        .'ممنوع إضافة أو حذف أو استبدال أو إعادة ترتيب أي مادة، وممنوع اختراع أسماء أو أرقام مواد. '
                        .'اكتب بالعربية باختصار وبأسلوب شخصي ومهني، واذكر العلاقة بين الصعوبة وأثر المادة في المسار. أعد JSON فقط حسب المخطط المطلوب.'
                    ]]],
                    'generationConfig' => [
                        'temperature' => 0.25,
