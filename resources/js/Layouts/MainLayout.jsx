    ...
    return (
        <div className={`min-h-screen transition-colors duration-500 flex flex-col font-sans ${isDark ? 'dark bg-[#0a0f18] text-white' : 'bg-[#fafcff] text-slate-900'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            ...
            <div className="fixed top-0 w-full z-[100] flex justify-center px-2 sm:px-4 transition-all duration-500 pointer-events-none">
                {/* 🔥 زيادة ارتفاع الناف بار لاستيعاب اللوجو الكبير 🔥 */}
                <nav className={`nav-capsule pointer-events-auto w-full max-w-[1400px] top-0 h-[90px] sm:h-[120px] bg-transparent border-b border-transparent ${scrolled ? 'nav-scrolled' : ''}`}>
                    <div className="h-full px-4 sm:px-6 lg:px-8 flex justify-between items-center">
                        ...
                        {/* 🔥 Logo Section (SUPER ENLARGED & Stacked Text) 🔥 */}
                        <Link href="/" className="flex items-center gap-4 group relative">
                            {/* 🔥 تكبير حاوية الصورة بشكل كبير جداً 🔥 */}
                        ...
                        <div className="flex items-center gap-2 sm:gap-3">
                            <button onClick={() => setLang(lang === 'ar' ? 'en' : 'ar')} className={`w-10 h-10 rounded-xl flex items-center justify-center text-[10px] font-black border transition-all active:scale-90 pointer-events-auto ${isDark ? 'bg-slate-800 border-white/10 text-indigo-400 hover:bg-slate-700' : 'bg-white border-slate-200 text-indigo-600 shadow-sm hover:bg-slate-50'}`}>
                                {lang === 'ar' ? 'EN' : 'AR'}
                            </button>
                            <button onClick={() => setIsDark(!isDark)} className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg transition-all active:scale-90 pointer-events-auto ${isDark ? 'bg-slate-800 text-yellow-400 border border-white/10 hover:bg-slate-700' : 'bg-white text-slate-400 border border-slate-200 shadow-sm hover:bg-slate-50'}`}>
                                {isDark ? '☀️' : '🌙'}
                            </button>
                            ...
                            {/* Mobile Menu Button */}
                            <button onClick={() => setMobileOpen(!mobileOpen)} className={`lg:hidden relative w-12 h-12 rounded-[1.25rem] flex items-center justify-center transition-all active:scale-95 z-[101] pointer-events-auto ${isDark ? 'bg-slate-800 text-white' : 'bg-white border border-slate-200 text-slate-700 shadow-sm'}`}>
                    ...
                    <div className="h-32 border-b border-white/10 flex items-center justify-between px-6 bg-gradient-to-r from-indigo-600 to-indigo-800 text-white">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col leading-none">
                                <span className="text-2xl font-black">سنفور</span>
                                <span className="text-sm font-black uppercase tracking-widest opacity-80">Sanfoor</span>
                            </div>
                        </div>
            ...
            {/* MAIN CONTENT */}
            {/* 🔥 زيادة الـ Padding العلوي لعدم اختفاء المحتوى تحت الناف بار الطويل 🔥 */}
            <main className="flex-1 flex flex-col w-full relative z-10 pt-28 sm:pt-40">
                {children}
            </main>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-12 border-b border-white/10 pb-16">
                        ...
                        <div className="md:col-span-5 space-y-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex flex-col leading-none">
                                    <h3 className="text-3xl font-black text-white pb-1">سنفور</h3>
                                    <span className="text-sm font-black text-indigo-400 tracking-[0.25em] uppercase">Sanfoor</span>
                                </div>
                            </div>
