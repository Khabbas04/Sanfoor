...
const filteredImportMajors = safeMajors.filter(m => m.college_id == fileData.college_id);
    ...
    const selectedMajor = useMemo(
        () => safeMajors.find(m => String(m.id) === String(data.major_id)) || null,
        [safeMajors, data.major_id]
    );

    const selectedPrerequisite = useMemo(
        () => safeCourses.find(c => String(c.id) === String(data.prerequisite_id)) || null,
        [safeCourses, data.prerequisite_id]
    );
    ...
    const previewTreeCourses = useMemo(() => {
        const targetMajorId = data.major_id === '' ? null : Number(data.major_id);

        const filtered = safeCourses
            .filter(course => targetMajorId === null
                ? course.major_id === null
                : course.major_id == targetMajorId || course.major_id === null
            )
            .sort((a, b) => {
        ...
        const previewSemester = Math.min(12, Math.max(1, Number(data.semester || 1)));
        const buckets = new Map();

        filtered.forEach(course => {
            const semesterKey = Math.min(12, Math.max(1, Number(course.semester || 1)));
            if (!buckets.has(semesterKey)) buckets.set(semesterKey, []);
            buckets.get(semesterKey).push(course);
        });

        if (!buckets.has(previewSemester)) buckets.set(previewSemester, []);

        buckets.get(previewSemester).push({
            id: editingCourse?.id || 'preview-course',
            name: data.name || 'المادة الجديدة',
            code: data.code || 'NEW000',
            type: data.type,
            credit_hours: data.credit_hours || 0,
            semester: previewSemester,
            minimum_passed_hours: data.minimum_passed_hours,
            __preview: true,
        });
        ...
        return Array.from(buckets.entries())
            ...
            .map(([semester, courses]) => ({
                ...
                courses: courses.sort((a, b) => {
                    if (a.__preview) return 1;
                    if (b.__preview) return -1;
    ...
    const nodeTone = (type, isPreview = false, isPrerequisite = false) => {
        if (isPreview) {
            return 'border-2 border-dashed border-indigo-400 bg-indigo-50 text-indigo-900 shadow-[0_0_0_3px_rgba(99,102,241,0.08)]';
        }

        if (isPrerequisite) {
            return 'border-emerald-300 bg-emerald-50 text-emerald-900 shadow-[0_0_0_2px_rgba(16,185,129,0.12)]';
        }

        switch (type) {
            case 'compulsory':
                return 'border-indigo-100 bg-white text-slate-800';
            case 'elective':
                return 'border-emerald-100 bg-emerald-50/60 text-emerald-900';
            case 'supporting':
                                        ...
                                        <div className="flex items-start justify-between gap-3">
                                            <div>
                                                <h4 className="text-[13px] font-black text-slate-800 flex items-center gap-2">
                                                    <span className="text-indigo-600">🌳</span> خريطة مصغرة لمكان المادة
                                                </h4>
                                                <p className="text-[11px] text-slate-500 font-bold mt-1">
                                                    المعاينة التالية توضّح مكان المادة داخل شجرة {selectedMajor ? selectedMajor.name : 'متطلبات الجامعة'} قبل الحفظ.
                                                </p>
                                            </div>
                                            <span className="text-[10px] font-black text-indigo-700 bg-indigo-100 px-2.5 py-1 rounded-full">
                                                الفصل {data.semester || 1}
                                            </span>
                                        </div>

                                        <div className="flex flex-wrap gap-2 text-[10px] font-black">
                                            <span className="px-2.5 py-1 rounded-full bg-white border border-slate-200 text-slate-600">
                                                {selectedMajor ? `التخصص: ${selectedMajor.name}` : 'خطة عامة بدون تخصص'}
                                            </span>
                                            <span className="px-2.5 py-1 rounded-full bg-indigo-100 text-indigo-700">المادة الجديدة</span>
                                            {selectedPrerequisite && (
                                                <span className="px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-700">
                                                    المتطلب: {selectedPrerequisite.code}
                                                </span>
                                            )}
                                        </div>

                                        <div className="overflow-x-auto pb-1">
                                            <div className="grid grid-flow-col auto-cols-[minmax(180px,1fr)] gap-3 min-w-max">
                                                {previewTreeCourses.map(({ semester, courses }) => (
                                                    <div key={semester} className={`rounded-2xl border p-3 ${Number(data.semester || 1) === semester ? 'border-indigo-300 bg-white shadow-sm' : 'border-slate-200 bg-white/80'}`}>
                                                        <div className="flex items-center justify-between mb-3">
                                                            <span className={`text-[11px] font-black px-2 py-1 rounded-full ${Number(data.semester || 1) === semester ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-600'}`}>
                                                                الفصل {semester}
                                                            </span>
                                                            <span className="text-[10px] font-bold text-slate-400">{courses.length} عقدة</span>
                                                        </div>

                                                        <div className="space-y-2">
                                                            {courses.map(course => {
                                                                const isPreview = Boolean(course.__preview);
                                                                const isPrerequisite = selectedPrerequisite && Number(selectedPrerequisite.id) === Number(course.id);

                                                                return (
                                                                    <div
                                                                        key={`${semester}-${course.id}-${isPreview ? 'preview' : 'existing'}`}
                                                                        className={`rounded-xl border px-3 py-2.5 transition-all ${nodeTone(course.type, isPreview, isPrerequisite)}`}
                                                                    >
                                                                        <div className="flex items-start justify-between gap-2">
                                                                            <div className="min-w-0">
                                                                                <p className="text-[11px] font-black leading-5 truncate">
                                                                                    {course.name}
                                                                                </p>
                                                                                <p className="text-[10px] font-mono font-bold opacity-70 mt-0.5" dir="ltr">
                                                                                    {course.code}
                                                                                </p>
                                                                            </div>
                                                                        ...
                                                                        <div className="mt-2 flex flex-wrap gap-1.5 text-[9px] font-black">
                                                                            {isPreview && <span className="px-1.5 py-0.5 rounded bg-indigo-600 text-white">جديد</span>}
                                                                            {isPrerequisite && <span className="px-1.5 py-0.5 rounded bg-emerald-600 text-white">متطلب</span>}
                                                                            {course.minimum_passed_hours ? (
                                                                                <span className="px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">
                                                                                    {course.minimum_passed_hours} ساعة
                                                                                </span>
                                                                            ) : null}
                                                                        </div>
                                                                    </div>
